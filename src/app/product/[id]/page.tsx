  ...
  return (
    <div className="product-page-content bg-bj-offwhite">
      ...
      {reviews && (() => {
        ...
        const totalPages = Math.ceil(reviews.reviews.length / REVIEWS_PER_PAGE)
        const pageReviews = reviews.reviews.slice(reviewPage * REVIEWS_PER_PAGE, (reviewPage + 1) * REVIEWS_PER_PAGE)
        return (
          <div className="bg-bj-offwhite py-20 border-t border-bj-gray-100">
            <div className="container-bj-wide">
              <LuxuryReveal direction="up">
                ...
                {pageReviews.map((review, index) => (
                  <LuxuryReveal key={review.id} direction="up" delay={index * 0.05}>
                    <div className="bg-white p-8 h-full flex flex-col">
                      ...
                      <p className="text-caption text-bj-gray-500 leading-relaxed mb-6 flex-1">{review.body}</p>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-3 mb-16">
                  <button
                    ...
                    className="w-9 h-9 flex items-center justify-center border border-bj-gray-200 text-bj-gray-400 hover:border-bj-black hover:text-bj-black transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  ...
                  {Array.from({ length: totalPages }).map((_, i) => (
                    <button
                      ...
                      className={`w-9 h-9 text-caption font-medium border transition-colors ${
                        i === reviewPage
                          ? 'border-bj-black bg-bj-black text-white'
                          : 'border-bj-gray-200 text-bj-gray-500 hover:border-bj-black hover:text-bj-black'
                      }`}
                      aria-label={`Page ${i + 1}`}
                    >
                      {i + 1}
              ...
              {/* Write a Review Form */}
              <LuxuryReveal direction="up">
                <div className="border-t border-bj-gray-200 pt-16">
                  <div className="max-w-xl">
                    <p className="text-overline text-bj-pink mb-3">Share Your Experience</p>
                    <h3 className="text-display-md text-bj-black mb-8">Write a Review</h3>

                    {reviewSubmitted ? (
                      <div className="bg-white p-8 flex items-start gap-4">
                        <CheckCircle2 size={22} className="text-emerald-500 flex-shrink-0 mt-0.5" />
                        <div>
                          <p className="text-caption font-semibold text-bj-black mb-1">Thank you for your review!</p>
                          <p className="text-caption text-bj-gray-500">Your feedback helps others discover the perfect piece.</p>
                          <button
                            onClick={() => setReviewSubmitted(false)}
                            className="mt-4 text-[12px] uppercase tracking-[0.1em] text-bj-pink underline underline-offset-4"
                          >
                            Write Another
                          </button>
                        </div>
                      ...
                      <form onSubmit={handleReviewSubmit} className="space-y-5">
                        <div>
                          <label className="block text-[11px] uppercase tracking-[0.12em] text-bj-gray-500 mb-2">Your Rating *</label>
                          <div className="flex items-center gap-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <button
                                key={star}
                              ...
                              >
                                <Star
                                  size={26}
                                  className={star <= reviewForm.rating ? 'text-[#C9A84C] fill-current' : 'text-bj-gray-300 hover:text-[#C9A84C]'}
                                />
                              </button>
                            ))}
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-[11px] uppercase tracking-[0.12em] text-bj-gray-500 mb-2">Name *</label>
                            <input
                              type="text"
                              value={reviewForm.name}
                              onChange={e => setReviewForm(f => ({ ...f, name: e.target.value }))}
                              placeholder="Your name"
                              className="w-full border border-bj-gray-200 px-4 py-3 text-caption text-bj-black placeholder-bj-gray-300 focus:outline-none focus:border-bj-black transition-colors bg-white"
                            />
                          </div>
                          <div>
                            <label className="block text-[11px] uppercase tracking-[0.12em] text-bj-gray-500 mb-2">Email *</label>
                            <input
                              type="email"
                        ...
                        <div>
                          <label className="block text-[11px] uppercase tracking-[0.12em] text-bj-gray-500 mb-2">Review Title *</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-[11px] uppercase tracking-[0.12em] text-bj-gray-500 mb-2">Your Review *</label>
                          <textarea
                            value={reviewForm.body}
                            onChange={e => setReviewForm(f => ({ ...f, body: e.target.value }))}
                            placeholder="Tell us about the quality, fit, and your overall experience..."
                            rows={5}
                            className="w-full border border-bj-gray-200 px-4 py-3 text-caption text-bj-black placeholder-bj-gray-300 focus:outline-none focus:border-bj-black transition-colors bg-white resize-none"
                          />
                        </div>

                        {reviewError && (
                          <p className="text-[12px] text-red-500">{reviewError}</p>
                        )}

                        <button
                          type="submit"
                          disabled={reviewSubmitting}
                          className="w-full sm:w-auto px-10 py-4 bg-bj-black text-white text-[12px] uppercase tracking-[0.12em] font-medium hover:bg-bj-gray-800 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                        >
