 ...
 * Served at /pinterest-feed.xml — RSS 2.0 with Google Base namespace
 * Pinterest accepts the same feed format as Google Merchant Center.
 * Required fields: id, title, description, link, image_link, price,
 *   availability, brand, product_type, google_product_category
 */
export async function GET(): Promise<Response> {
  ...
  const items = productGroups.flatMap(product =>
    product.variants.map(variant => {
      ...
      <g:product_type>Jewelry &gt; Lab-Grown Diamond Jewelry</g:product_type>
